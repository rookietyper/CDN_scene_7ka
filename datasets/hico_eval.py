...
class HICOEvaluator():
    ...
    def compute_map(self):
        ...
        print('--------------------')
        print( len(ap_buy))
        print("buy map{}".format(sum(ap_buy) / len(ap_buy)))
        print('mAP: {} mAP rare: {}  mAP non-rare: {}  mean max recall: {}'.format(m_ap, m_ap_rare, m_ap_non_rare, m_max_recall))
        print('--------------------')

        return {'mAP': m_ap, 'mAP rare': m_ap_rare, 'mAP non-rare': m_ap_non_rare, 'mean max recall': m_max_recall, 'buy_mAP': sum(ap_buy) / len(ap_buy)}
