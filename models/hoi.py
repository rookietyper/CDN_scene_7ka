...
class CDNHOI(nn.Module):
    ...
    def forward(self, samples: NestedTensor, background = None):
        ...
        hopd_out, interaction_decoder_out = self.transformer(self.input_proj(src), mask, self.query_embed.weight, pos[-1],background)[:2]
...
class SetCriterionHOI(nn.Module):
    ...
    def loss_obj_labels(self, outputs, targets, indices, num_interactions, log=True):
        ...
        if self.amp:
            loss_obj_ce = F.cross_entropy(src_logits.transpose(1, 2), target_classes, obj_weights.half())
        else:
...
class PostProcessHOI(nn.Module):
    ...
    def forward(self, outputs, target_sizes,background = None):
        ...
        if self.only_use_mask:
            ...
            scene_mask = torch.ones((bs, 117))
            for i in range(bs):
                if background[i] == 0:
                    scene_mask[i, 7] = 0
            scene_mask = scene_mask.unsqueeze(1).repeat( 1, 64, 1)
            # print(outputs_verb_class[1, 1, 21, 7])
            verb_scores = verb_scores * scene_mask.cuda()
